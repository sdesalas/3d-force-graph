...
export default class ForceGraph3D {
	...
	update() {
		...
		const highlightMaterial = new THREE.SpriteMaterial( { color: 0xffffff, map: sprite, blending: THREE.AdditiveBlending, depthTest: false, transparent : true } );
		...
		this.graphData.nodes.forEach(node => {
			...
			const material = new THREE.SpriteMaterial( { color: color, opacity: opacity, map: sprite, blending: THREE.AdditiveBlending, depthTest: false, transparent : true } );
		...
		this.graphData.links.forEach(link => {
			...
			const line = new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: color, transparent: true, opacity: Math.abs(opacity * 0.3) }));
